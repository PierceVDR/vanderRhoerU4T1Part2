public class WhileLoopFun {
    ...
    public boolean isPrime(int number) {
        ...
        int currentFactor = 1; // The number itself is always a factor

        // We can stop at half of number, since the only factor after that is number itself, which we've already counted.
        while (currentFactor<=number/2 && numOfFactors<3) { // Only need to know if there are 1, 2, or 3+ factors, so we can stop at 3
            int rem = number % currentFactor;

            if (rem==0) {numOfFactors++;}
